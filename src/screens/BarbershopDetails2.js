...
export default function BarbershopDetails2({ navigation, route }) {
    ...
    async function getProfile() {
        ...
        await axios({
            ...
            const data1 = {
                data: response.data[0].barbershop[0]
            }
            setData(data1)
            for (let j = 0; j < response.data[0].barbershop[0].favorites.length; j++) {
                if (response.data[0].barbershop[0].favorites.id === parseInt(userID)) {
                    response.data[0].barbershop[0].favorite = true
                    setisLiked(response.data[0].barbershop[0].favorite)
                }
            }
            setLoading(false)
            console.log(data.data.photo)
        }).catch((e) => {
            console.log('ERROR ON GET PROFILE: ', e.response.data)
        })
    }
